...
trun_hurdle <- function(psi, theta, lambda, unit_length, word, max_length) {
  prob_list <- c()
  y_list <- c()
  yp <- psi
  prob_list <- c(prob_list, yp)
  y_list <- c(y_list, 0)
...
nb_psi_s <- extract(gen_pois_s_hurdle, pars = c("psi"))
...
for (i in 1:4000) {
  ...
  max_length <- max(nb_length)
  for (length_val in nb_length) {
    nb_place <- c(nb_place, trun_hurdle(nb_psi_s$psi[i], nb_theta_s$theta1[i], 
